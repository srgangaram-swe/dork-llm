# AxiomStack — developer workflow commands.
...
.PHONY: test-e2e
test-e2e: ## Run the Playwright browser integration suite
	npm run test:e2e

.PHONY: check
check: lint format-check typecheck test test-web ## Run local Python + web quality gates
...
train-frontier: ## Compatibility alias: train the modern-small DorkLLM profile
...
.PHONY: train-modern-small
train-modern-small: train-frontier ## Train the RoPE/RMSNorm/SwiGLU/GQA profile
...
.PHONY: web
web: ## Run strict model-backed DorkChat + API
	uvicorn apps.api:app --reload --host 127.0.0.1 --port 8790

.PHONY: web-demo
web-demo: ## Run DorkChat with the explicitly labeled offline demo provider
	DORK_DEMO_MODE=1 DORK_MODEL_PROVIDER=mock uvicorn apps.api:app --reload --host 127.0.0.1 --port 8790
...
clean: ## Remove caches and build artifacts
	rm -rf .pytest_cache .mypy_cache .ruff_cache build dist *.egg-info htmlcov .coverage coverage.xml test-results playwright-report blob-report
